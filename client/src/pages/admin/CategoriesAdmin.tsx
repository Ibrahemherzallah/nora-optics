              ...
              <table className="w-full text-sm">
                ...
                {data?.map((c) => (
                    <tr key={c._id} className="border-b border-line last:border-0">
                      ...
                      <td className="p-3 text-right text-muted">{c.description || '—'}</td>
                      <td className="p-3">
...
function CategoryModal({ form, onClose }: { form: CategoryForm; onClose: () => void }) {
  ...
  const validate = (s: CategoryForm): Record<string, string> => {
    const e: Record<string, string> = {};
    if (!s.name.trim()) e.name = 'الاسم مطلوب';
    if (!s.image) e.image = 'الصورة مطلوبة';
    return e;
  };
  ...
  // Upload straight to Firebase Storage from the browser, store the URL.
  const uploadImage = async (file: File) => {
    setUploading(true);
    setError(null);
    try {
      const url = await uploadImageToFirebase(file, 'category_images');
      ...
      <Modal title={state._id ? 'تعديل صنف' : 'صنف جديد'} onClose={onClose} maxWidth="max-w-lg">
        <div className="space-y-4">
          <div>
            ...
            <input
                className={errors.name ? 'input border-destructive focus:ring-destructive/30' : 'input'}
                value={state.name}
                onChange={(e) => { setState({ ...state, name: e.target.value }); clearError('name'); }}
            />
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              ...
              <label className={`flex h-16 w-16 cursor-pointer items-center justify-center rounded-xl border-2 border-dashed hover:bg-surface ${errors.image ? 'border-destructive' : 'border-line'}`}>
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <button className="btn-primary" disabled={save.isPending || uploading} onClick={handleSave}>
