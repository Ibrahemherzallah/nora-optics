            ...
            <tbody>
              {data?.data?.map((p: any) => (
                <tr key={p._id} className="border-b border-line last:border-0">
                  ...
                  <td className="nums p-3 text-muted">{p.code}</td>
                  <td className="nums p-3">{shekel(p.price)}</td>
                  <td className="nums p-3 text-muted">{shekel(p.cost)}</td>
                  <td className="p-3">
                    {p.isSoldOut && <span className="rounded-full bg-red-100 px-2 py-0.5 text-xs text-destructive">نفذت</span>}
                  </td>
                  <td className="p-3">
  ...
  const uploadImages = async (colorIdx: number, files: FileList) => {
    ...
    try {
      ...
      setState((s) => {
        ...
        colors[colorIdx] = { ...colors[colorIdx], images: [...colors[colorIdx].images, ...data.urls] };
        return { ...s, colors };
      });
    } catch (e) {
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input className="input nums" value={state.code} onChange={(e) => setState({ ...state, code: e.target.value })} />
            </div>
          </div>
          <div className="grid grid-cols-3 gap-4">
            <div>
              <label className="label">السعر *</label>
              <input className="input nums" type="number" value={state.price} onChange={(e) => setState({ ...state, price: +e.target.value })} />
            </div>
            <div>
              <label className="label">التكلفة *</label>
              <input className="input nums" type="number" value={state.cost} onChange={(e) => setState({ ...state, cost: +e.target.value })} />
            </div>
            <div>
                ...
                <div key={idx} className="rounded-xl border border-line p-3">
                  <div className="flex items-center gap-2">
                    ...
                    {state.colors.length > 1 && (
                      <button
                        className="rounded-lg p-2 text-destructive hover:bg-red-50"
                        onClick={() => setState((s) => ({ ...s, colors: s.colors.filter((_, i) => i !== idx) }))}
                      >
                        <Trash2 size={16} />
                      </button>
                    )}
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-2">
                    ...
                    <label className="flex h-14 w-14 cursor-pointer items-center justify-center rounded-lg border-2 border-dashed border-line hover:bg-surface">
                      ...
                      <input
                        ...
                        multiple
                        className="hidden"
                        onChange={(e) => e.target.files && uploadImages(idx, e.target.files)}
                      />
                    </label>
                  </div>
                </div>
          ...
          <label className="flex items-center gap-2 text-sm">
            <input type="checkbox" checked={state.isSoldOut} onChange={(e) => setState({ ...state, isSoldOut: e.target.checked })} className="accent-lime" />
            نفذت الكمية
          </label>
